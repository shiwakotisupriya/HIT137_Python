def characterEncript(character, shift1, shift2):
    """Encrypts a character using custom shift rules based on case and position."""
...
def characterDecript(character, rule, shift1, shift2):
    """Reverses encryption based on the applied rule."""
    shifts = {
...
def file_encription(fileInput, fileOutput, filerules, shift1, shift2):
    """Encrypts file content and saves encryption rules."""
    try:
        ...
        print(f"Encryption is complete and output is saved to '{fileOutput}'.")
        return True
...
def fileDecription(fileInput, filerules, fileOutput, shift1, shift2):
    """Decrypts file content using saved encryption rules."""
    try:
        with open(fileInput, 'r', encoding='utf-8') as f:
            content = f.read()
...
def verify_decryption(original_file, decrypted_file):
    """Verifies decryption accuracy by comparing files."""
    try:
...
def main():
    """Main execution flow for encryption system."""
    ...
    if not file_encription("raw_text.txt", "encrypted_text.txt", "encryption_rules.txt", shift1, shift2):
        return
    if not fileDecription("encrypted_text.txt", "encryption_rules.txt", "decrypted_text.txt", shift1, shift2):
        return
